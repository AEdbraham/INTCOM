...
const CommentCard = ({ index, commentData, leftVal = 0 }) => {
    ...
    const handleReplyClick = () => {
        if(!access_token){
            return toast.error("Inicia sesión primero para dejar un comentario");
        }
            ...
            <div className="my-5 p-6 rounded-md border border-grey">
                ...
                <div className="flex gap-5 items-center mt-5">
                    
                    {
                        ...
                        <button onClick={loadReplies} className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2"><i className="fi fi-rr-comment mt-1"></i> {children.length} Responder </button>
                        :
                        <button onClick={hideReplies} className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2"><i className="fi fi-rr-comment mt-1"></i> Esconder Respuesta </button>
                    }
